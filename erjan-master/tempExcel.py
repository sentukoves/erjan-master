...
def index():
    ...
    if re.method == "GET":
        if id_get and not re.args.get("abyte"):
            collection = QeryStatsFileParse(id_get).filterOrf
            return render_template("index.html", slovar=collection, datetimenow=datetime.datetime.now().date(), flag=0 , fl=id_get)

        if re.args.get("abyte"):
            arr = str(re.args.get("abyte"))
            print(id_get)
            parse_array(
                "INSERT INTO Speller (Name) VALUES ('{}')".format(arr))
